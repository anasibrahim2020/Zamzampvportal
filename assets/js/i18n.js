...
const I18N_EN = {
  ...
  'الإعداد غير مكتمل — لازم تضيف بيانات Supabase الأول (راجع دليل الإعداد).':'Setup incomplete — Supabase credentials are required first (see the setup guide).',
  ...
  'هيتم حذف إثبات التحويل من كل طلبات المجموعة وفك ارتباطها ببعض. الطلبات نفسها مش هتتأثر، وتقدر ترفع إثبات جديد بعدها.':'The transfer proof will be removed from every request in the group and the grouping will be undone. The requests themselves are not affected, and you can upload a new proof afterwards.',
  ...
  'إضافة فاتورة مورّد مكتملة (اسم المورّد ورقم الفاتورة والمبلغ)':'add a complete supplier invoice (supplier name, invoice no. and amount)',
  'إضافة فاتورة العميل ومبلغها (أو تفعيل «فاتورة عامة»)':'add the client invoice and its amount (or enable "General Invoice")',
  'إدخال إجمالي المبلغ المطلوب صرفه':'enter the total amount to pay',
  ...
  'لا يمكن إلغاء هذا الطلب.\n\nالمبيعات تقدر تلغي الطلب فقط قبل اعتماد الحسابات.':'This request cannot be cancelled.\n\nSales can only cancel a request before accounts approval.',
  ...
  'لازم تسجل دخول أولاً.':'Please sign in first.',
  ...
  'تجهيز نسخة PDF من الطلب بيستخدم شاشة النموذج مؤقتاً، وده هيمسح البيانات المكتوبة حالياً في النموذج وغير المحفوظة.':'Preparing the request PDF temporarily uses the form screen, which will clear the unsaved data currently typed into the form.',
  ...
  'كل حاجة تمام 👌':'You are all caught up 👌',
